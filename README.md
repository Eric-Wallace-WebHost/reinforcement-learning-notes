...
[Q-Prop: Sample-Efficient Policy Gradient with an Off-Policy Critic](https://arxiv.org/abs/1611.02247) rather than learn both the Critic and Actor in an off-policy manner like ACER, they only learn the critic off-policy. When using a state-dependent baseline like in most Actor-Critic algorithms, we can't learn the critic off-policy because V is V(pi), aka it depends on the policy that is running to get V. The Q-Prop paper and [this blog](https://www.alexirpan.com/rl-derivations/) provide derivations for an unbiased action-dependent baseline. This baseline should be better as it takes into account both state and action. It is learned off-olicy. Important note: Q-Prop is an approach for estimating the gradient. It says nothing about how to apply it. You can directly add the gradient with SGD, or you can use natural policy gradient, or TRPO.
